...
export const blogPageType = defineType({
  ...
  fields: [
    ...
    defineField({ name: 'heroTitleEn', title: 'Hero — Заглавие (EN)', type: 'string' }),
    defineField({ name: 'heroTitleGold', title: 'Hero — Златна дума', type: 'string' }),
    defineField({ name: 'heroTitleGoldEn', title: 'Hero — Златна дума (EN)', type: 'string' }),
    defineField({ name: 'heroSubtitle', title: 'Hero — Подзаглавие', type: 'text', rows: 3 }),
    defineField({ name: 'heroSubtitleEn', title: 'Hero — Подзаглавие (EN)', type: 'text', rows: 3 }),
    defineField({ name: 'ctaTitle', title: 'Призив — Заглавие', type: 'string' }),
    defineField({ name: 'ctaTitleEn', title: 'Призив — Заглавие (EN)', type: 'string' }),
    defineField({ name: 'ctaSubtitle', title: 'Призив — Подзаглавие', type: 'text', rows: 2 }),
    defineField({ name: 'ctaSubtitleEn', title: 'Призив — Подзаглавие (EN)', type: 'text', rows: 2 }),
  ],
