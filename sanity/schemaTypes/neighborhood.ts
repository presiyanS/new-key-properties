...
export const neighborhoodType = defineType({
  ...
  fields: [
    ...
    defineField({ name: 'pros', title: 'Предимства', type: 'array', of: [{ type: 'string' }], options: { layout: 'tags' } }),
    defineField({ name: 'cons', title: 'Недостатъци', type: 'array', of: [{ type: 'string' }], options: { layout: 'tags' } }),
