...
type StatusFilter = 'all' | 'active' | 'under_offer' | 'sold'
...
export default function ListingsClient({ listings, phone, phoneDisplay, email, bottomCtaTitle, bottomCtaSubtitle }: Props) {
  ...
  useEffect(() => {
    ...
    if (sp.get('status')) setStatusFilter(sp.get('status') as StatusFilter)
  ...
  }, [filter, categoryFilter, searchQuery, neighborhood, rooms, priceMin, priceMax, areaMin, areaMax, constructionAct, statusFilter, sortBy, showFilters, pathname])
        ...
        {showFilters && (
          <div className="bg-white border border-gray-200 rounded-2xl p-5 mb-5 shadow-xs">
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
              ...
              {/* Status */}
              <div>
                <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">{dict.listings.statusLabel}</label>
                <div className="relative">
                  <select
                    value={statusFilter}
                    ...
                    className={selectClass + ' w-full'}
                  >
                    <option value="all">{dict.listings.allStatuses}</option>
                    <option value="active">{dict.listings.statusActive}</option>
                    <option value="under_offer">{dict.listings.statusUnderOffer}</option>
                    <option value="sold">{dict.listings.statusSold}</option>
                  </select>
                  <svg className="absolute right-2.5 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 pointer-events-none" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                  </svg>
