...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (locale === 'en') {
    const title = `${name} Neighborhood, Sofia — Prices, Rent & Listings | New Key Properties`
    const description = n.metaDescriptionEn ?? `Full guide to the ${name} neighborhood in Sofia: current property prices, rents, transport, pros and cons. New Key Properties.`
    return {
      title,
      description,
      keywords: `${name} neighborhood, ${name} Sofia properties, ${name} apartments, ${name} rent, buy property ${name}`,
      openGraph: {
        title,
        description,
...
export default async function NeighborhoodPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const name = isEn ? (n.nameEn ?? n.name) : n.name
  const tagline = isEn ? (n.taglineEn ?? n.tagline) : n.tagline
  const transport = isEn ? (n.transportEn ?? n.transport) : n.transport
  const targetAudience = isEn ? (n.targetAudienceEn ?? n.targetAudience) : n.targetAudience
  const pros: string[] = (isEn && n.prosEn?.length ? n.prosEn : n.pros) ?? []
  const cons: string[] = (isEn && n.consEn?.length ? n.consEn : n.cons) ?? []

  const descriptionText = isEn ? (n.descriptionEn ?? n.description) : n.description
  const paragraphs = descriptionText?.split('\n\n').filter(Boolean) ?? []
  const rawFaqs: { question: string; questionEn?: string; answer: string; answerEn?: string }[] = n.faq ?? []
  ...
  const jsonLd: Record<string, unknown>[] = [
    {
      ...
      '@type': 'Place',
      name: isEn ? `${name} Neighborhood, Sofia` : `Квартал ${name}, София`,
      description: (isEn ? n.metaDescriptionEn : n.metaDescription) ?? (isEn ? `Information about the ${name} neighborhood in Sofia.` : `Информация за квартал ${name} в София.`),
      url: `https://newkey.bg${localizeHref(`/kvartali/${slug}`, locale)}`,
      ...(n.externalImageUrl ? { image: n.externalImageUrl } : {}),
      containedInPlace: { '@type': 'City', name: 'Sofia', addressCountry: 'BG' },
    },
  ]
      ...
      <section className="relative h-80 lg:h-96 bg-brand-green overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 right-0 pb-10 px-4 sm:px-6 lg:px-8">
          <div className="max-w-7xl mx-auto">
            ...
            <h1 className="font-serif text-5xl lg:text-6xl font-bold text-white">{name}</h1>
            {tagline && <p className="text-white/70 text-xl mt-2">{tagline}</p>}
          </div>
        </div>
      </section>
            ...
            {transport && (
              <div className="bg-white rounded-2xl p-8 shadow-xs border border-gray-100">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <p className="text-gray-600 leading-relaxed">{transport}</p>
                </div>
              </div>
            )}
            ...
            {neighborhoodListings.length > 0 && (
              <div>
                <h2 className="font-serif text-2xl font-bold text-brand-green mb-6">
                  {dict.neighborhoods.listingsHeadingPrefix} {name}
                </h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                {neighborhoodListings.length > 4 && (
                  <div className="text-center mt-6">
                    ...
                    >
                      {dict.neighborhoods.viewAllListingsPrefix} {neighborhoodListings.length} {dict.neighborhoods.viewAllListingsMiddle} {name}
                    </Link>
                  </div>
                )}
              </div>
